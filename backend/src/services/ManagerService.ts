...
export class ManagerService {
  ...
  public async getBanners(type: string): Promise<any[]> {
    try {
      ...
      throw new InternalServerErrorException("Erro ao buscar a lista de banners.");
    }
  }
  ...
  public async updateBannerLink(id: number, linkUrl: string): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException("Erro ao atualizar o link do banner.");
    }
  }
   ...
   */
  public async reorderBanners(banners: { id: number; order_index: number }[]): Promise<void> {
    ...
    try {
      ...
      throw new InternalServerErrorException("Erro ao salvar a nova ordem dos banners.");
    }
  }
  ...
  public async postBanner(
    ...
    if (!files || files.length === 0) {
      throw new UnprocessableEntityException("Nenhum arquivo de imagem foi enviado.");
    }

    const folder = `shared/${type}`;
    const validationResult = await this.validateImages(files, folder);
    const result: { invalidImages: string[]; urlImages: string[]; failedUrls?: string[] } =
      validationResult;
    ...
    if (result.urlImages.length > 0) {
      ...
      try {
        ...
        if (body.order) {
          ...
          await this.publicRepository.shiftOrderIndices(type, currentOrder, numNewBanners);
        } else {
        ...
        try {
          ...
          this.logger.error(
            `Erro ao persistir banners em massa: ${bulkError}. Tentando fallback um a um.`,
          );
        ...
        throw new InternalServerErrorException("Erro ao salvar dados dos banners.");
      }
    }
  ...
  public async validateImages(
    ...
    try {
      const { validImages, invalidImages } = await this.validateProductFiles(images);
   ...
   */
  private async convertToWebp(images: Express.Multer.File[]): Promise<Express.Multer.File[]> {
    return Promise.all(
      images.map(async (image) => {
        if (["image/avif", "image/heif", "image/heic"].includes(image.mimetype)) {
          return image;
        }
        ...
        const finalName = isWebp ? image.originalname : `${originalNameWithoutExt}.webp`;
  ...
  public async postCategory(category: string): Promise<any> {
    ...
    try {
      ...
      throw new InternalServerErrorException(`Erro ao adicionar categoria: ${error}`);
    }
  }
  ...
  private async validateCategoryForPost(category: string): Promise<boolean> {
    ...
    if (!normalizedCategory || normalizedCategory.length < 2) {
      throw new BadRequestException("A categoria deve ser um texto com pelo menos 2 caracteres.");
    }
    ...
    if (alreadyExists) {
      throw new ConflictException(`A categoria "${normalizedCategory}" já está cadastrada.`);
    }
   ...
   * @private
   */
  private async validateColorForPost(name: string, color: string): Promise<boolean> {
    // 1. Limpeza e validação básica
    const normalizedName = name?.trim();

    if (!normalizedName || normalizedName.length < 2) {
      throw new BadRequestException("O nome da cor deve ser um texto com pelo menos 2 caracteres.");
    }
    ...
    if (alreadyExists1) {
      throw new ConflictException(`A cor com o nome "${normalizedName}" já está cadastrada.`);
    }

    const alreadyExists2 = colors.some((c) => c.color.toLowerCase() === color.toLowerCase());

    if (alreadyExists2) {
      throw new ConflictException(`A cor com a cor "${color}" já está cadastrada.`);
    }
