...
export class DiscountService {
  ...
  public async processAutomaticDiscounts(isStartup: boolean = false) {
    this.logger.log(`Iniciando análise de descontos (Modo Startup: ${isStartup})`);
    ...
    try {
      if (isStartup) {
        ...
        if (hasActive && lastRotation && !this.isDateStale(lastRotation, 2)) {
          this.logger.log("Promoções vigentes detectadas. Nenhuma rotação necessária no startup.");
          return;
        }
      }
      ...
      const allProducts = await this.productRepository.getProductsEligibilityData();

      // Filtra candidatos que cumprem a regra de idade (60 dias)
      const candidates = allProducts.filter((p) => this.isEligibleByAge(p.created_at));
      ...
      const productsStockMap = await this.productRepository.getProductsTotalStock(candidateIds);
      ...
      this.logger.log(`Processo finalizado. ${discountsToApply.length} novos descontos aplicados.`);
    } catch (error) {
      this.logger.error("Falha no processamento autônomo de descontos", error.stack);
      throw error;
    }
  }
  ...
  public async applyDiscount(productId: number, discountPercentage: number) {
    try {
      ...
      this.logger.error(`Erro ao aplicar desconto no produto ${productId}: ${error.message}`);
      throw new InternalServerErrorException("Erro interno ao processar a regra de desconto.");
    }
  }
