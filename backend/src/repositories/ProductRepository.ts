...
export class ProductRepository {
	...
	public async clearAllDiscounts(): Promise<void> {
		...
		try {
			await db.run(query);
			this.logger.log(
				"Todos os descontos foram removidos da tabela 'discounts'.",
			);
		} catch (error) {
			this.logger.error(`Erro ao limpar todos os descontos: ${error}`);
			throw new InternalServerErrorException(
				"Erro ao limpar descontos existentes.",
			);
		}
	}
    ...
    `;
		try {
			...
			this.logger.log(
				`Desconto salvo para o produto ${discountData.product_id}`,
			);
		} catch (error) {
			this.logger.error(
			...
			throw new InternalServerErrorException(
				"Erro ao salvar registro de desconto.",
			);
		}
	}
	...
	> {
		...
		try {
			...
			throw new InternalServerErrorException(
				"Erro ao buscar dados de elegibilidade.",
			);
		}
	}
	...
	public async getTotalStock(productId: number): Promise<number> {
		...
		try {
			...
			this.logger.error(
				`Erro ao obter estoque total para o produto ${productId}: ${error}`,
			);
			throw new InternalServerErrorException(
				"Erro ao obter estoque total do produto.",
			);
		}
	}
	...
	public async getDiscountedProducts(
		...
		for (let i = 0; i < productIds.length; i += chunkSize) {
			...
			try {
				...
				throw new InternalServerErrorException(
					"Erro ao consultar histórico de descontos em lote.",
				);
			}
		}
	 ...
	 */
	public async getProductsTotalStock(
		productIds: number[],
		...
		for (let i = 0; i < productIds.length; i += chunkSize) {
			...
			try {
				...
				throw new InternalServerErrorException(
					"Erro ao obter estoque total dos produtos.",
				);
			}
		}
		return stockMap;
	 ...
	 */
	public async hasProductBeenDiscountedBefore(
		productId: number,
	): Promise<boolean> {
		const db = await this.getDatabase();
		...
		try {
			...
			throw new InternalServerErrorException(
				"Erro ao consultar histórico de descontos.",
			);
		}
	}
	...
	public async cleanupOldDiscountsHistory(): Promise<void> {
      ...
      WHERE data_desconto NOT IN (
		...
		try {
			...
			this.logger.error(
				`Erro ao limpar histórico antigo de descontos: ${error}`,
			);
		}
	}
	...
	public async getLastRotationDate(): Promise<string | null> {
		...
		const result = await db.get(
			"SELECT MAX(data_desconto) as lastRun FROM old_discounts",
		);
	...
	public async updateProductBasicInfo(
		...
		try {
			...
			throw new InternalServerErrorException(
				"Erro ao atualizar informações básicas do produto.",
			);
		}
	}
	...
	public async deleteProductUrls(productId: number): Promise<void> {
		...
		try {
			...
			this.logger.error(
				`Erro ao deletar URLs do produto ID ${productId}: ${error}`,
			);
			throw new InternalServerErrorException(
				"Erro ao deletar mídias do produto.",
			);
		}
	}
	...
	public async deleteProductColors(productId: number): Promise<void> {
		...
		try {
			...
			this.logger.error(
				`Erro ao deletar cores/variantes do produto ID ${productId}: ${error}`,
			);
			throw new InternalServerErrorException(
				"Erro ao deletar variações do produto.",
			);
		}
	}
