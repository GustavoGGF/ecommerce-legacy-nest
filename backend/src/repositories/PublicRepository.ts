...
export class PublicRepository {
  ...
  public async getBannersByType(type: string): Promise<any[]> {
    ...
    return await db.all("SELECT * FROM banners WHERE type = ? ORDER BY order_index ASC", [type]);
  }
  ...
  public async deleteBanner(id: number): Promise<any> {
    const db = await this.getDatabase();
    const banner = await db.get("SELECT image_url FROM banners WHERE id = ?", [id]);
  ...
  public async updateBannerOrder(id: number, orderIndex: number): Promise<void> {
    const db = await this.getDatabase();
    await db.run("UPDATE banners SET order_index = ? WHERE id = ?", [orderIndex, id]);
  }
  ...
  public async getMaxOrder(type: string): Promise<number> {
    const db = await this.getDatabase();
    const result = await db.get("SELECT MAX(order_index) as maxOrder FROM banners WHERE type = ?", [
      type,
    ]);
   ...
   */
  public async getLowStockProducts(threshold: number, limit: number): Promise<any[]> {
    const db = await this.getDatabase();
  ...
  public async getProductImageCount(productId: number): Promise<number> {
    const db = await this.getDatabase();
    const result = await db.get("SELECT COUNT(*) as total FROM product_urls WHERE product_id = ?", [
      productId,
    ]);
    return result?.total || 0;
  }
  ...
  public async getProductById(id: number): Promise<any> {
    ...
    const images = await db.all(`SELECT url FROM product_urls WHERE product_id = ? ORDER BY id`, [
      id,
    ]);
