...
export class CatalogRepository {
	...
	public async getCategories(): Promise<any[]> {
		...
		try {
			...
			throw new InternalServerErrorException(
				`Erro ao buscar o catálogo: ${error}`,
			);
		}
	}
	...
	public async getCategoryById(categoryId: string): Promise<any> {
		...
		try {
			...
			this.logger.error(
				`Erro ao buscar categoria por ID ${categoryId}: ${error}`,
			);
			throw new InternalServerErrorException(
				`Erro ao buscar categoria por ID ${categoryId}`,
			);
		}
	}
	...
	public async updateCategoryUrl(id: string, url: string): Promise<void> {
		...
		try {
			...
			this.logger.error(
				`Erro ao atualizar URL da categoria no banco: ${error}`,
			);
			throw new InternalServerErrorException(
				`Erro ao atualizar URL da categoria: ${error}`,
			);
		}
	}
}
