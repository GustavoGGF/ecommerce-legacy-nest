...
export class SearchIndexRepository {
   ...
   */
  public async getVariantDetailsForIndex(productColorId: number): Promise<any | null> {
  ...
  /**
   * Obtém os detalhes de múltiplas variantes de produto para indexação.
   */
  public async getVariantDetailsForIndexBulk(productColorIds: number[]): Promise<any[]> {
    ...
    for (let i = 0; i < productColorIds.length; i += chunkSize) {
      const chunk = productColorIds.slice(i, i + chunkSize);
      ...
      const chunkResults = await db.all(sql, chunk);
  ...
  public async upsertSearchIndex(data: any): Promise<void> {
    ...
    await db.run("DELETE FROM products_search_index WHERE product_color_id = ?", [
  ...
  /**
   * Insere ou atualiza registros no índice de busca FTS5 em lote.
   */
  public async upsertSearchIndexBulk(dataArray: any[]): Promise<void> {
    if (!dataArray || dataArray.length === 0) {
      return;
    }

    const db = await this.getDatabase();
    const chunkSize = 100; // Limits the number of bindings per query (8 * 100 = 800 bindings)
    ...
    for (let i = 0; i < dataArray.length; i += chunkSize) {
      ...
      const deleteIds = chunk.map((data) => data.product_color_id);
      await db.run(
        `DELETE FROM products_search_index WHERE product_color_id IN (${deletePlaceholders})`,
        deleteIds,
      );

      // 2. Bulk Insert
      const insertPlaceholders = chunk.map(() => "(?, ?, ?, ?, ?, ?, ?, ?)").join(",");
      const insertValues = chunk.flatMap((data) => [
        data.product_color_id,
        data.product_id,
        data.price,
        data.nome,
