...
export class AuthController {
   ...
   */
  private setRefreshTokenCookie(res: Response, rawToken: string, expiresAt: Date): void {
    const cookieName = this.configService.get<string>("REFRESH_TOKEN_COOKIE") || "refresh_token";
  ...
  @ApiResponse({
    status: 200,
    description: "Login bem-sucedido. Retorna os dados do usuário e o access_token.",
  })
  @ApiResponse({
    status: 401,
  ...
  async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const cookieName = this.configService.get<string>("REFRESH_TOKEN_COOKIE") || "refresh_token";
    const rawToken: string | undefined = (req.cookies as Record<string, string>)?.[cookieName];
    if (rawToken) {
