...
export class BannerController {
  ...
  @ApiOperation({
    ...
    description: "Retorna banners ativos e inativos para listagem no painel administrativo.",
  })
  ...
  @Put("link/:id")
  public async updateBannerLink(@Param("id") id: number, @Body("link_url") linkUrl: string) {
    return await this.service.updateBannerLink(id, linkUrl);
  }

  @ApiOperation({
    summary: "Reordena a lista de banners",
    description: "Recebe uma lista de banners com seus novos índices de ordenação.",
  })
  @ApiResponse({
    status: 200,
  ...
  @Put("reorder")
  public async reorderBanners(@Body() banners: { id: number; order_index: number }[]) {
