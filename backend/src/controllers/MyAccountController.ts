...
export class MyAccountController {
  ...
  @ApiOperation({
    ...
    description: "Retorna vazio, apenas para o HTML ser gerado no interceptor/pipe",
  })
  @ApiResponse({
    status: 200,
  ...
  public async getValidationManagement(@Res() res: Response): Promise<any> {
    const indexPath = path.join(process.cwd(), "client", "browser", "index.csr.html");

    res.sendFile(indexPath, (err) => {
      if (err && !res.headersSent) {
        this.logger.error(`Erro ao enviar index.csr.html: ${err.message}`);
        res.status(404).send("Template Base (CSR) não encontrado. Verifique o build do frontend.");
      }
    });
  }
  ...
  @ApiOperation({
    ...
    description: "Valida os dados do produto e processa até 5 arquivos de imagem simultaneamente.",
  })
  ...
  @ApiOperation({
    ...
    description: "Atualiza os metadados do produto, reconstrói as mídias e vincula cores/tamanhos.",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: "Valida os dados da cor e processa até 5 arquivos de imagem simultaneamente.",
  })
  @ApiConsumes("multipart/form-data")
  @ApiResponse({
    status: 201,
  ...
  public async postColor(@Body() body: { name: string; color: string }): Promise<any> {
  ...
  @ApiOperation({
    ...
    description: "Vincula uma URL de imagem a uma categoria específica no catálogo.",
  })
  @ApiResponse({
    status: 200,
